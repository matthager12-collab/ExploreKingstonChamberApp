...
| 6c | **Set up the weekly member-export routine**: save a GrowthZone report (business name, membership status, level, drop date, categories, address — NO member emails/personal contacts) and export CSV/Excel weekly + before board meetings, delivered to Mat or uploaded to the app importer when it ships. Agent-automatable later per ADR-0002. | Chamber GrowthZone staff login |
