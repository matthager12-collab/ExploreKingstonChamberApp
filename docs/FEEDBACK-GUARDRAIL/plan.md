...
| T-10 | `src/app/(site)/admin/feedback/comment-list.tsx`: a "rewritten" marker on `moderated` rows | T-05 | The marker is text, not a glyph alone, and is announced by a screen reader. Rows without the flag are visually unchanged. |
...
| T-17 | Tell the Chamber, before deploy, that the notice bump re-prompts every returning visitor for consent | — | Written notice sent, so the consent-rate dip is not read as a bug (DEC-007). |
