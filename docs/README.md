...
|---|---|---|
| [STATUS-2026-08.md](STATUS-2026-08.md) | **August 2026 state-vs-plan review** — evidence-based audit of the running code against the v1 spec (69/71 FRs), ROADMAP-V2 (3 of 4 P0s actually shipped), and all 13 coalition epics (3%–55% done, avg ~24%), plus the docs-drift ledger in both directions | Planning what to build next; correcting stale doc claims |
| [COMPLETION-PLAN.md](COMPLETION-PLAN.md) | The plan to close all 13 epics and 112 coalition stories — 73 catalogued work items sequenced by real gates: Track 0 external clocks (GBP API, LTAC pre-clearance, Resend config), Wave 1 unblocked code-only work, Wave 2 externally gated, Wave 3 commerce | Sequencing work; starting any epic |
| [CODE-REVIEW-2026-08.md](CODE-REVIEW-2026-08.md) | 45 adversarially-verified build findings across 7 dimensions — 3 high (takedown no-op on seed records, inert ISR on home//ferry, 52k-row scan on the request path), 20 medium, 22 low — each with file:line and a fix sketch | Fixing quality debt; before enabling the forecast |
| [FERRY-FORECAST-REFIT/](FERRY-FORECAST-REFIT/plan.md) | **The plan that acts on that evaluation** — four phases (measure honestly → refit the fallback → make the data primary → probability-of-full), 20 tasks, 11 decided one-way doors in [decisions.md](FERRY-FORECAST-REFIT/decisions.md), runnable exit criteria in [verification.md](FERRY-FORECAST-REFIT/verification.md), and an [adversarial review](FERRY-FORECAST-REFIT/review.md) whose two confirmed findings reopened a decision round | Executing the forecast fix; before touching `CURVES`, the empirical blend, or the accuracy verdict |
| [FERRY-FORECAST-ACCURACY-2026-08.md](FERRY-FORECAST-ACCURACY-2026-08.md) | **Ferry busyness accuracy evaluation** measured against 2,156 real sailings (50 days of prod `ferry_observation`) — the reported 32-pt MAE is ~57% a per-snapshot grading artifact; graded per sailing the model runs 22 pts LOW, barely beating a constant, and a cross-validated refit of the same data reaches ~11 pts MAE / 60% level match. 9 ranked changes with measured effect | Before enabling the forecast; retuning `CURVES` or the empirical blend |
