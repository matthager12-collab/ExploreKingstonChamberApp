...
| Facebook Events | https://developers.facebook.com/docs/graph-api/reference/event/ | Public events API removed 2018; scraping violates ToS | n/a | output channel only — **not** an ingest source. Members bring their own post's text instead: "Paste from Facebook or Instagram" in the business portal reads it into a draft event they confirm (`src/lib/events/extract-post.ts`, Claude Haiku 4.5, `ANTHROPIC_API_KEY`). Nothing fetches Meta. |
| Instagram (business/creator accounts) | https://developers.facebook.com/docs/instagram-platform | Graph API can read an account's OWN captions once that account authorises us. Standard Access covers accounts with a role on our Meta app (i.e. the Chamber's); every other member needs Advanced Access via App Review — business verification, privacy policy, data-deletion endpoint, screencast, 2–4 weeks | Free (API); metered LLM cost to read captions | **not built** — candidate automated source, needs a decision with Mat first (would be the first relaxation of SOURCE_ALLOWLIST; promotional posts rank below every existing source, so it amends ADR-0002 precedence too) |
