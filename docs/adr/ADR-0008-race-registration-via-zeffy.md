...
- **Two headers loosen, by one origin each.** CSP gains `frame-src 'self' https://www.zeffy.com` — `'self'` restated because declaring `frame-src` replaces the `default-src` fallback, and Admin → Site content frames the site's own pages as a preview. Permissions-Policy `payment=()` becomes `payment=(self "https://www.zeffy.com")`, so Apple Pay and Google Pay work inside the frame. `self` is required: a page can only delegate a feature it holds itself. Measured in Chrome — with Zeffy alone in the list, the frame had payment disabled and Google Pay logged 17 policy violations; adding `self` cleared them. The app has no payment code, so holding the permission here is inert. Pinned by `tests/unit/security-headers.test.ts`.
