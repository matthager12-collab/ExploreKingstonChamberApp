...
- **Click to load.** Zeffy's embed page loads Microsoft Clarity session recording, Amplitude, Google Tag Manager and ad cookies. The privacy page promises "no third-party analytics or ad tech", so the frame is created only when a visitor presses **Register now**; until then nothing from Zeffy loads, which is where the plain link left things. A note under the button says Zeffy sets its own cookies and analytics. The plain link stays below as "Or open the form on zeffy.com".
- **Two headers loosen, by one origin each.** CSP gains `frame-src 'self' https://www.zeffy.com` — `'self'` restated because declaring `frame-src` replaces the `default-src` fallback, and Admin → Site content frames the site's own pages as a preview. Permissions-Policy `payment=()` becomes `payment=("https://www.zeffy.com")`, so Apple Pay and Google Pay work inside the frame; this origin itself stays denied. Pinned by `tests/unit/security-headers.test.ts`.
...
- **Only Zeffy is framed.** `registrationEmbedUrl` in `src/lib/data/race.ts` must be on `https://www.zeffy.com`; the component refuses anything else and falls back to the link, and the CSP would block it regardless.
- **No `sandbox` attribute.** Zeffy's checkout runs Stripe, 3-D Secure and wallet sheets in the frame; a sandbox that broke one of them would only surface on a real purchase.
