...
- **Two headers loosen, by one origin each.** CSP gains `frame-src https://www.zeffy.com`. Permissions-Policy `payment=()` becomes `payment=("https://www.zeffy.com")`, so Apple Pay and Google Pay work inside the frame; this origin itself stays denied. Pinned by `tests/unit/security-headers.test.ts`.
