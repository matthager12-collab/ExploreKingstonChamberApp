...
| 17 | Side-of-water framing via a `vk-side` **cookie** + a geo divide at -122.44 lng | Reframe for Kingston-side vs Edmonds-side visitors, user-initiated location only (amended: the original opt-out auto-ask was removed for the MHMDA consent floor), no account needed | Two separate sites; a required account/profile; re-detecting on every request |
