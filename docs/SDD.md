...
| `blob-store.ts` | `hasR2()` = all four `R2_IMAGES_*` set; else `hasBlob()` = `BLOB_READ_WRITE_TOKEN` set | image bytes written under `.data/…` by the domain store | **R2 (E15, preferred):** `putObject`/`getObject`/`deleteObject` against a PRIVATE bucket over the S3 API (`aws4fetch` SigV4). Keys MIRROR the disk layout, and the value stored on the record stays the same fs-relative string filesystem mode uses — so migration is a pure byte copy with **zero record rewrites**, and the app's own image routes proxy every read. **Vercel Blob (legacy):** `putImage()` returns a full public CDN URL. Callers store a **string** either way and hand it to `<img src>`, *no branch in callers*. |
| `image-sanitize.ts` | always — called at all four save choke points | n/a (pure, no backend) | `stripImageMetadata(bytes, type)` removes EXIF/XMP/IPTC — GPS above all — from JPEG/PNG/WebP/GIF/HEIC without re-encoding pixels (M-16-02, child-safety floor). **Fail-closed:** throws `UnstrippableImageError` on anything it cannot fully parse, and the upload routes turn that into a 4xx. HEIC is stripped by zeroing the Exif item's extent *in place*, so every `iloc` offset stays valid and the coded image is untouched by construction. |
...
- **`near-me.tsx`** — geolocation. `NearMePlace[]` (serializable Restaurant subset). State `idle→consent→locating→ready|denied|error`; **E11: an affirmative-consent card precedes the browser prompt** (version-gated against `PRIVACY_NOTICE_VERSION` via `src/lib/privacy/consent.ts`; declining sets `denied`, whose fallback keeps the walk-time ordering — nothing is lost). On grant it stores `vk-consent-geo` and sends ONE `consent` analytics event (session id + notice version, no location). One `getCurrentPosition` per tap; sorts by client haversine; sends at most one geo-ping (a `useRef` latch), coords rounded before leaving the device and **stored server-side as the area bucket only**.
