...
import { bayPivot, transformRing } from "@/lib/map/bay-transform";
...
// ---- Port bay layer (E34) ----------------------------------------------
//
// A PRIVATE palette, deliberately NOT an extension of PARKING_RULE_COLORS.
// Two reasons, and the second is the load-bearing one.
//
// 1. The bay layer answers a different question. A zone's colour says what the
//    RULE is; a bay's colour says WHICH CODE YOU TEXT. The Port's own map is
//    organised entirely around that second question — yellow/green/grey for
//    POKPARK/POKHILL/POKTT — and collapsing all three into one `paid` purple
//    is exactly what made our map unreadable.
// 2. `ParkingRule` is hand-synced across seven files and pinned by
//    tests/unit/parking-rule-palette.test.ts, which asserts ΔE76 ≥ 30 between
//    every pair in the legend. Adding `port-use` and `disabled` to that union
//    to serve one layer would force a seven-file change and re-open a solved
//    colour-separation problem for hues that are not even rules.
//
// Values are Explore Kingston brand tokens from globals.css: tide-deep, fern
// and coral for the three pay codes. free-2hr green and permit taupe are the
// SAME values the zone palette already uses, so a bay and its zone agree.
// MEASURED, not picked by eye. Each value is the nearest colour to its brand
// token that clears BOTH floors below; the drift from the token is ΔE76 2.5–8.7,
// which is not visible side by side.
//
//   1. WCAG 1.4.11 — ≥3:1 against every base surface in ADR-0007's PALETTE.
//      This is why the tokens themselves could not be used raw: tide-deep
//      #16758f measures 2.80:1 on greenspace #aac4a4 and fern #4a7c59 measures
//      2.58:1. Both FAIL. Bays are small filled shapes a visitor has to pick out
//      against parkland, so this floor is not negotiable.
//   2. ΔE76 ≥ 30 from each other, so the three pay codes never blur together —
//      that separation is the entire reason this layer exists.
//
// OPEN, and deliberately left visible rather than buried: judged against the
// FULL parking legend these do not all clear 30. POKPARK sits ΔE 15.4 from
// free-unrestricted #1E96C0 and POKHILL ΔE 26.0 from permit #7a7468. They are
// drawn in a different FORM (small filled bays vs. a street stroke and a zone
// wash) and ADR-0007 §Context says to judge overlay-against-base on hue, value
// and chroma together rather than one number — but that argument is weaker
// inside one flat legend than it is on the canvas. See the ADR-0007 amendment;
// this is the call to review.
const STALL_COLORS: Record<string, string> = {
  "paid:POKPARK": "#096f8a", // tide-deep, darkened to 3.05:1
  "paid:POKHILL": "#3f6f4e", // fern, darkened to 3.10:1
  "paid:POKTT": "#9c511e", // coral, darkened to 3.09:1
  "free-2hr": "#2e9e4f", // unchanged — matches PARKING_RULE_COLORS["free-2hr"]
  permit: "#7a7468", // unchanged — matches PARKING_RULE_COLORS.permit
  disabled: "#33395d", // sound, deepened off ferry-holding navy
  // Port-use and KCYC reuse `permit` rather than taking a seventh hue. Both
  // mean "you need something you do not have", which is exactly what permit
  // already says, and the legend was oversubscribed before this layer added
  // anything. The Port's own map splits them; a visitor's decision does not.
  "port-use": "#7a7468",
};
const FALLBACK_STALL_COLOR = "#7a7468";

const STALL_LABELS: Record<string, string> = {
  "paid:POKPARK": "General vehicle · text POKPARK",
  "paid:POKHILL": "Hill · text POKHILL",
  "paid:POKTT": "Truck & trailer · text POKTT",
  "free-2hr": "Free · 2-hour limit",
  permit: "Marina tenant / KCYC · permit",
  disabled: "Disabled parking",
  "port-use": "Port use / KCYC — not public",
};

/** A bay's palette key: pay code when it has one, else the bare rule. */
function stallKey(rule: string, code: string | null | undefined): string {
  return code ? `${rule}:${code}` : rule;
}

/** One bay as generated by scripts/gen-port-stalls.py. */
...
export function FeatureMap({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Port bay polygons (E34). Their own layer, not `fills`: they are drawn
      // opaque with a hairline divider so individual stalls read, while `fills`
      // is translucent zone wash — putting them together would either wash out
      // the bays or blacken the zones.
      const portStalls: GeoJSON.Feature[] = [];
      ...
      const draw = async () => {
        ...
        // ---- built-ins: Port bay geometry (fetched) --------------------
        // Same contract as streets: flagged server-side, static file fetched
        // here, failure is silent because the zone polygons underneath already
        // carry the answer. The per-zone nudge rides on the view payload and is
        // applied at draw time rather than baked into the file, so an admin
        // correction shows up on the next revalidation with no regeneration.
        if (view.builtins.portStalls) {
          try {
            const res = await fetch("/geo/port-stalls.json");
            if (res.ok && !cancelled && mapRef.current === map) {
              const data = (await res.json()) as { features: StallFeature[] };
              const transforms = view.builtins.portStalls.transforms ?? {};
              // Pivot per zone, computed from that zone's own bays, so a
              // rotation turns the row about its own centre instead of
              // swinging it around the lot. Grouped first for exactly that.
              const byZone = new Map<string, StallFeature[]>();
              for (const f of data.features ?? []) {
                const zone = f.properties?.zone;
                if (!zone || f.geometry?.type !== "Polygon") continue;
                const list = byZone.get(zone);
                if (list) list.push(f);
                else byZone.set(zone, [f]);
              }
              for (const [zone, bays] of byZone) {
                ...
                for (const b of bays) {
                  ...
                  const key = stallKey(b.properties.rule, b.properties.code);
                  const color = STALL_COLORS[key] ?? FALLBACK_STALL_COLOR;
                  const label = STALL_LABELS[key] ?? key;
                  const range = b.properties.range;
                  portStalls.push({
                    type: "Feature",
                    properties: {
                      color,
                      popup: `<div style="font-size:0.8rem;line-height:1.35;max-width:230px;">
                        <p style="margin:0;font-weight:600;font-size:0.95rem;">${esc(
                          range ? `Spaces ${range}` : label,
                        )}</p>
                        <p style="margin:4px 0 0;font-weight:600;color:${color};">${esc(label)}</p>
                        <p style="margin:4px 0 0;">Bay outlines follow the Port's map, not a survey — the painted lines on the ground are the authority.</p>
                      </div>`,
                    },
                    geometry: {
                      type: "Polygon",
                      coordinates: [
                        pivot && t ? transformRing(ring, pivot, t) : ring,
                      ],
                    },
                  } as GeoJSON.Feature);
                }
              }
              // One legend row per colour actually drawn — the legend has to
              // explain the pay codes, which is the whole point of the layer.
              // Added after the loop so it reflects what rendered, and keyed by
              // palette key so addLegend's dedupe collapses the repeats.
              for (const f of data.features ?? []) {
                const key = stallKey(f.properties?.rule, f.properties?.code);
                if (!STALL_COLORS[key]) continue;
                addLegend({
                  key: `stall-${key}`,
                  label: STALL_LABELS[key] ?? key,
                  color: STALL_COLORS[key],
                  shape: "swatch",
                });
              }
            }
          } catch {
            // Same reasoning as streets: the zone fills still answer the
            // question, so a missing bay layer degrades rather than breaks.
          }
        }
        ...
        // Port bays go down FIRST, under every other overlay: they are the
        // finest-grained thing on the map and the zone wash, curb strokes and
        // pins all need to read on top of them. Opaque fill with a hairline
        // divider in the same hue — that divider is what makes 302 bays read as
        // rows of individual stalls rather than one coloured slab, and it is
        // the single detail that makes this look like the Port's own map.
        if (portStalls.length) {
          addGeo("fm-stalls", portStalls);
          map.addLayer({ id: "fm-stalls", type: "fill", source: "fm-stalls", paint: { "fill-color": ["get", "color"], "fill-opacity": 0.9 } });
          map.addLayer({ id: "fm-stalls-divider", type: "line", source: "fm-stalls", paint: { "line-color": "#ffffff", "line-width": 0.6, "line-opacity": 0.55 } });
          wirePopup("fm-stalls");
        }
        if (fills.length) {
