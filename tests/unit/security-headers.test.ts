...
describe("next.config security headers", () => {
  ...
  it("keeps the Payment Request API off everywhere, the Zeffy frame included", async () => {
    // payment was briefly delegated to Zeffy's registration frame on /race for
    // Apple Pay and Google Pay. Zeffy does not show either on embedded forms
    // (support.zeffy.com, "Supported Payment methods"), so the delegation
    // bought nothing and was reverted on 2026-09-22 (ADR-0008 amendment 1).
    const rules = await loadRules();
    const pp = headerMap(rules.find((r) => r.source === "/(.*)")!).get("Permissions-Policy")!;
